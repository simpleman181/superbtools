...
export default function Navbar() {
  ...
  return (
    <header ref={ref} className="sticky top-0 z-50 bg-[#1a3c5e] text-white shadow-md">
      <div className="mx-auto max-w-7xl px-4">
        <div className="flex h-14 items-center gap-2">
          ...
          <Link href="/" onClick={closeAll}
            className="flex items-center gap-1.5 font-bold text-base flex-shrink-0 text-white hover:text-orange-300 transition-colors mr-2">
            <span className="text-orange-400 text-lg">⚡</span>
            <span className="hidden sm:inline">MyToolMate</span>
          </Link>

          {/* Desktop nav — fills remaining space, buttons shrink to fit */}
              ...
              return (
                <div key={cat.id} className="relative">
                  <button
                    ...
                    className={`flex items-center gap-0.5 px-2 py-1.5 text-xs font-medium rounded transition-colors whitespace-nowrap ${
                      isOpen
                  ...
                  {/* Dropdown */}
                  {isOpen && (
                    <div className={`absolute top-full mt-1 rounded-xl border border-gray-200 bg-white shadow-2xl z-[200] ${
                      isWide ? "w-[480px] left-0" : "w-52 left-0"
                    }`}>
          ...
          <div className="flex items-center gap-1 flex-shrink-0 ml-auto">
            ...
            <div className="relative">
              ...
              {showSearch && (
                <div className="absolute right-0 top-full mt-2 w-72 rounded-xl border border-gray-200 bg-white shadow-2xl z-[200]">
                  <div className="p-2">
                    <input
                      ref={searchRef}
                      value={query}
                    ...
                    <div className="px-2 pb-2 max-h-64 overflow-y-auto">
                      {results.length === 0 ? (
                        ...
                        <Link key={t.href} href={t.href} onClick={closeAll}
                          ...
                          <span>{t.name}</span>
                          <span className="text-xs text-gray-400 ml-2 flex-shrink-0">{t.category}</span>
                        </Link>
                      ))}
                    </div>
                  )}
                  {query.length <= 1 && (
                    <p className="text-xs text-gray-400 text-center pb-3">Type to search</p>
                  )}
                </div>
              )}
            </div>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden bg-white text-gray-800 border-t border-white/20 max-h-[80vh] overflow-y-auto">
          {/* Mobile search */}
